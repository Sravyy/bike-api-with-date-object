...
$(document).ready(function() {
  ...
  $('#formOne').submit(function(e) {
    ...
    $.ajax({
      url: `https://bikeindex.org:443/api/v3/search?location=${location}&distance=100&stolenness=proximity&per_page=5`,
      type: 'GET',
      data: {
        format: 'json'
      },
      success: function(response) {
        ...
        response.bikes.forEach(function(item) {
          ...
          if (item.date_stolen <= currentDate && item.date_stolen >= pastweekDate) {
            ...
            var n = d.toUTCString();
            $('#output').append(`${item.title}`+"<br>"+`${n}`+"<br>"+`${item.stolen_location}`+"<br>"+ "<img src='"+`${item.thumb}`+"' />"+"<br>");
          }
        });
      },
      error: function() {
